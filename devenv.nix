  ...
  git-hooks = lib.mkIf (!isCI) {
    ...
    hooks = {
      ...
      "push:test" = {
        ...
        entry = "${pkgs.bash}/bin/bash -c 'set -e; ROOT=\"$DEVENV_ROOT\"; ${pkgs.fvm}/bin/fvm flutter test \"$ROOT/verily_app/test\" && ${pkgs.fvm}/bin/fvm flutter test \"$ROOT/verily_ui/test\" && cd \"$ROOT/verily_core\" && ${config.env.DEVENV_PROFILE}/bin/dart test'";
        pass_filenames = false;
        always_run = true;
        stages = [ "pre-push" ];
      };
